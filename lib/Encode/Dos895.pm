...
The module implements the Kamenicky encoding for Czech, popular in the
early nineties.

You can use either of the following names as the first parameter to C<encode> and C<decode>:
...
while (<DATA>) {
    ...
    $from = chr hex $from;
    $to = eval qq("\\N{$to}");
    $FROM .= $from, $TO .= $to if $from ne $to;
}
my $decode = eval qq( sub { tr{$FROM}{$TO} } );
my $encode = eval qq( sub { tr{$TO}{$FROM} } );
...
__PACKAGE__->Define(qw( dos-895 nec-867 keybcs2 ));
